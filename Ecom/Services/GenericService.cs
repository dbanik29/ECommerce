...
{
    public class GenericService<Tbl_Entity> : IService<Tbl_Entity> where Tbl_Entity : class
    {
        DbSet<Tbl_Entity> _dbSet;
        private dbMyOnlineShoppingEntities _dbMyShopEntity;

        public GenericService(dbMyOnlineShoppingEntities dbMyShopEntity)
        {
            _dbMyShopEntity = dbMyShopEntity;
            _dbSet = _dbMyShopEntity.Set<Tbl_Entity>();
        }

        public void Add(Tbl_Entity entity)
        {
            _dbSet.Add(entity);
            _dbMyShopEntity.SaveChanges();
        }
        ...
        public IEnumerable<Tbl_Entity> GetResultBySqlprocedure(string query, params object[] parameters)
        {
            if (parameters != null)
            {
                return _dbMyShopEntity.Database.SqlQuery<Tbl_Entity>(query, parameters).ToList();
            }
            else
                return _dbMyShopEntity.Database.SqlQuery<Tbl_Entity>(query).ToList();
        }
        ...
        {
            if (_dbMyShopEntity.Entry(entity).State == EntityState.Detached)
        ...
        {
            _dbSet.Attach(entity);
            _dbMyShopEntity.Entry(entity).State = EntityState.Modified;
            _dbMyShopEntity.SaveChanges();
        }
